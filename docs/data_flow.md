...
- 出力処理: 骨格描画済みフレームを PreviewWindow へ表示し、同時に正規化座標でOSカーソル移動とピンチジェスチャークリックを実行
- 失敗時挙動: 推論セッション生成失敗時は NoopProcessor へフォールバックし、実行中ワーカーエラー時はランドマーク状態をリセットして継続
...
graph TD
    ...
    F -->|try_send frame| G
    G -->|landmarks| F
    G -->|error| N[State reset and continue]
    N -->|recover| F
    F -->|draw skeleton + cursor target| I[Processed Frame]
    F -->|move_cursor_normalized| J[OS Cursor]
    F -->|detect pinch gesture| O[trigger_mouse_click]
    O -->|mouse event| P[OS Click]
...
実運用の主経路は `CameraSession -> FrameProcessor -> PreviewWindow` です。推論処理はワーカー分離され、UI更新をブロックしない構成です。加えて、`FrameProcessor` はカーソル移動とクリック判定を同一フレーム処理内で統合します。
