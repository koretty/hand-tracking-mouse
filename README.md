...
既存のハンドトラッキング環境は、セットアップが重い・依存関係が複雑・挙動確認まで時間がかかるという課題がありました。
本プロジェクトでは、「ハンドトラッキング検証の初速が遅い」という課題に対し、Rust製の軽量なパイプラインとONNX Runtimeによる推論実行を組み合わせることで、迅速に検証を開始できる環境の構築を目的としています。

競合と比べた強みは、以下の4点です。

1. Rustベースで処理フローを明確に分割し、実験コードから実運用コードへ拡張しやすい設計としている。
2. カメラの列挙・選択・設定保存までを内包し、再起動時の再設定コストを削減できる。
3. 推論失敗時にNoop処理へフォールバックすることで、開発中の停止リスクを抑制している。
4. 用途に合わせて設計した軽量な自作モデルを採用しており、推論速度とリソース使用量の最適化を図っている。
